...
- `string`, `float`, `long`, `ulong`, `int`, `uint`, `short`, `ushort`, `byte`, `sbyte`, `bool`, `double`, `char`, `DateTime`, `MyTime`, `TimeSpan`, enum
- 위 타입의 배열, 중첩 객체와 `ValueTuple`

`MyTime`은 UTC ticks, `TimeSpan`은 duration ticks를 부호 있는 64비트 정수로 기록한다. 두 타입을 포함하는 페이로드는 송수신 양쪽에 해당 타입을 지원하는 직렬화기가 필요하다.
...
컬렉션 등 목록에 없는 타입은 지원된다고 가정하지 않는다. `MyNetDeserializer`는 패키지 페이로드용이며 임의의 신뢰하지 않는 Stream을 처리하는 범용 역직렬화기로 사용하지 않는다.
