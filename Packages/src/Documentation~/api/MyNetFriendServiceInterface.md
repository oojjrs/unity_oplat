...
현재 Anonymous, Steam, Ugsymous에서 친구 목록 조회·반복 조회·추가·초대 발송과 수신을 지원한다.
...
Ugsymous는 UGS Player ID로 친구 요청을 보낸다. 대상의 요청이 이미 들어와 있으면 친구 관계를 수락하고, 이미 친구이거나 같은 대상에게 보낸 요청이 있으면 성공으로 처리한다. 자기 자신을 지정하면 `NotPermitted`로 완료한다.
...
`OnOk(roomId, playerId)`는 플랫폼으로 발송 요청을 넘겼다는 뜻이며 상대에게 도착했거나 상대가 수락했다는 보장은 없다. Anonymous는 접속한 대상에게 즉시 전달하고 오프라인 초대를 보관하지 않는다. Steam과 Ugsymous의 발송 결과 역시 전달 확인으로 해석하지 않는다.

Steam은 호출자가 현재 참가한 Lobby와 `RoomId`가 같고 대상이 현재 Steam 친구일 때 `InviteUserToLobby`를 호출한다. Steam이 요청을 거절하면 `NotPermitted`로 완료한다.

Ugsymous는 호출자가 현재 참가한 Session과 `RoomId`가 같고 대상이 현재 친구이면서 UGS presence가 `Online`, `Busy`, `Away` 중 하나일 때 Friends 메시지로 초대를 보낸다. Friends 메시지는 오프라인 저장을 지원하지 않는다.
...
빈 대상 ID는 `EmptyPlayerId`, 빈 방 ID는 `EmptyRoomId`, 사라진 방은 `NotFoundRoom`으로 처리한다. 해당 방의 구성원이 아니거나 플랫폼에서 요청을 처리할 수 없는 경우는 `NotPermitted`다. Anonymous에서 초대 대상이 미접속인 경우, Steam에서 대상 ID·친구 관계·Overlay·현재 Lobby 조건이 맞지 않는 경우, Ugsymous에서 대상이 친구가 아니거나 presence가 초대 가능한 상태가 아닌 경우도 `NotPermitted`다. 미접속은 친구 추가를 거절하는 이유가 아니다. 플랫폼 예외와 서버 저장 실패는 `OnException`으로 전달한다.
...
공식 API: [Friends 시작하기](https://docs.unity.com/en-us/friends/guides/get-started), [Presence](https://docs.unity.com/en-us/friends/concepts/presence), [Events](https://docs.unity.com/en-us/friends/concepts/events).
