...
친구 추가·초대는 아직 구현하지 않았다. 저장된 파일이 없으면 친구 추가 구현 전까지 목록은 비어 있다. 실행 인스턴스는 같은 버전을 사용해야 하며, 이번에 계정 연결 메시지에 Project Key·App ID가 추가되었으므로 이전 버전의 로컬 서버는 종료한 뒤 다시 실행한다.
