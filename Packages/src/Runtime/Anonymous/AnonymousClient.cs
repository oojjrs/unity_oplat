...
namespace oojjrs.oplat.anonymous
{
    internal sealed class AnonymousClient
    {
        ...
        internal bool TryReceiveChat(out byte[] content)
        {
            if ((_messages != null) && _messages.TryReceive(message => message.Type == AnonymousTransport.Message.TypeEnum.ChatReceived, out var message))
            {
        ...
        internal bool TryReceiveFriendInvite(out byte[] content)
        {
            if ((_messages != null) && _messages.TryReceive(message => message.Type == AnonymousTransport.Message.TypeEnum.FriendInvited, out var message))
            {
                content = message.Content;
                return true;
            }
